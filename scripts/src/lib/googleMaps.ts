...
export async function loadGoogleMaps(libs: string[] = ['places']) {
  ...
  if (!ready) {
    ready = (async () => {
      ...
      try { await (google as any).maps.importLibrary?.('maps') } catch (e: any) { try { console.error('GMAPS import maps error:', e?.message || e) } catch {} }
      try { if (libs.includes('places')) await (google as any).maps.importLibrary?.('places') } catch (e: any) { try { console.error('GMAPS import places error:', e?.message || e) } catch {} }
